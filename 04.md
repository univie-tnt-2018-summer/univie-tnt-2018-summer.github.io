...
**NB**: the commands below are from the tutorial on *Programming historian*, but they are, as we discovered today are outdated and are not likely to work on your machines with the latest version of `pandoc` and `LaTeX` (MikTex Package).
...
The following commands, however, definitely work: I have tested them on a fresh installation on Windows. *NB:* On Windows, you may see a pop-up Windows from `MikTex` asking to download a package for `LaTeX`. This means that some package is missing and you need to download it (one or several!). Uncheck a birdie to install all necessary ones at once. After that everything should work.
